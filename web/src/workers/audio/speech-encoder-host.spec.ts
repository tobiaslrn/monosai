...
function fakeCodecs(
  options: {
    packets?: number;
    failOn?: 'configure' | 'output';
    onConfig?: (config: AudioEncoderConfig) => void;
  } = {},
) {
  const packets = options.packets ?? 2;
  class FakeAudioEncoder {
    static isConfigSupported = vi.fn((config: AudioEncoderConfig) => {
...
describe('SpeechEncoderHost', () => {
  it('answers an encode with compressed bytes, handed over rather than copied', async () => {
    let supportedConfig: AudioEncoderConfig | undefined;
    const codecs = fakeCodecs({ onConfig: (config) => (supportedConfig = config) });
    const { host, posted, transfers } = harness(codecs);

    await host.handleMessage(encodeMessage());

    expect(supportedConfig).toMatchObject({ codec: 'opus', bitrate: 96_000 });
