...
const TASK_LAYER = [
  ...
  'Success criteria:',
  '- Return at most one finding per supplied sentence ordinal. Prefer a construction above the supplied profile ceiling; otherwise choose only the single most pedagogically useful construction.',
  ...
  '- Use the sentence order as context only for disambiguation. Findings must remain attached to the ordinal of the sentence where the grammar occurs.',
  ...
  'Output semantics: `findings` contains `{ sentenceId, label, explanationEn, confidence, inProfile, spanJa }`; sentenceId is the supplied ordinal, and confidence is "low", "medium", or "high" and states how sure you are the construction is present and correctly named.',
] as const;
...
export function buildGrammarPrompt(request: GrammarReviewRequest): AssembledPrompt {
  ...
  const user = assemble([
    asConfig('grammar profile', grammarProfileSection(request)),
    asData(
      'sentences',
      markdownIndexedLines(
        'Sentences',
        request.sentences.map((sentence, index) => ({
          index: grammarSentenceWireId(index),
          value: sentence.textJa,
        })),
      ),
    ),
  ]);
...
/** Ordinal ids are cheaper and keep domain sentence ids off the provider wire. */
export function grammarSentenceWireId(index: number): string {
  return String(index);
}

function grammarProfileSection(request: GrammarReviewRequest): string {
  return markdownDocument([
    markdownHeading(1, 'Grammar profile'),
    markdownField('Register', request.registerPreference),
    markdownDocument([markdownHeading(2, 'Guidance'), markdownLineValue(request.profileGuidance)]),
  ]);
}
