...
const context = await browser.newContext({ storageState: state, viewport: { width: 393, height: 851 }, serviceWorkers: 'block' });
...
await page.evaluate(() => new Promise((resolve, reject) => {
  ...
  request.onsuccess = () => {
    ...
    rows.onsuccess = () => {
      const titles = ['A New Bicycle', 'The Library Window', 'A Walk in the Mountains', 'The Night Sky', 'A Special Cup of Tea', 'The Little Garden', 'A Rainy Afternoon', 'On the Way Home'];
      titles.forEach((title, index) => store.put({ ...rows.result[0], id: index === 0 ? rows.result[0].id : crypto.randomUUID(), title, lastOpenedAt: index % 3 === 0 ? Date.now() : null, createdAt: Date.now() - (index < 2 ? 0 : index < 4 ? 1 : 4) * 86400000 }));
    };
