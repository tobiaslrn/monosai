...
job line and every failure is unchanged, and the live region carries all of it.
...
| State | Control |
| --- | --- |
| A run is going | **Stop generating audio**, with the coverage drawn as a ring around it |
| Idle with a partial set | **Generate audio**, the count in its tooltip |
...
- A run in progress adds **no words** to the card. The generation fill breathes while requests are open and the ring around the contextual control fills as clips land; a line saying "4 of 13 sentences ready" said what both were already saying. A run can be stopped from that control as well as from **Stop generating audio** in the reader's overflow menu, which stays beside **Delete audio** and matches **Stop translating**.
- The **track is a slider** over the sentences of the reading, so a reading can be aimed at rather than only stepped through. It snaps sentence to sentence, and a position with no clip snaps to the nearest one that has, so a hole is somewhere the drag passes over. The thumb then lands where playback landed rather than where the drag was released, and a drop that reaches nothing playable leaves it where it was: the thumb, the position line, and the sound are one position or the drop did nothing. Releasing it in a live session jumps there and keeps reading; releasing it while nothing is playing only moves the cursor, because starting a reading is still the press of Play. It is disabled until something has audio.
...
- The announcement reads "Sentence 4 of 13" while playing, "13 sentences ready" while idle with the whole reading prepared, "Not playing" while there is still something to prepare, "Waiting for sentence 5 of 13" at the frontier, and "Finished" at the end of a reading, followed by whatever the contextual control is about. While waiting, the play control is disabled and named for what it is doing: the session has already started and there is nothing to press. A finished reading is not a reset — the cursor stays on the last sentence, the track stays full, and the play control is named **Play again**.
- Generation reports how many sentences are ready rather than which one it is at, because four requests are open at once and there is no single sentence the run has reached. A run that stopped reports the *reading's* ready-over-total, which is the pair the track beside it is drawing, and says what the run itself managed as an attempt — "This attempt covered 0 of the 4 sentences it set out to read." A run count phrased as though it were the reading read as "nothing is ready" beside a bar showing that a third of it was.
- **Try again is withdrawn once it has been shown not to work.** A sentence the configured voice cannot read fails the same way every attempt, and each attempt costs a request per missing sentence, so after two consecutive attempts that produced no audio the control becomes **Dismiss** and the announcement says that trying again produced nothing and that a different voice or model may read those sentences. An attempt that produces anything, or a change of voice, restores the offer.
...
- **Leaving the reader ends the session** (ADR 0041). The player is the only transport there is, and the Library has no banner, no mini-player, and no control whose name mentions audio — so playback that outlived the route was a sound a learner could not reach. Going back stops it, clears the cursor, and leaves every clip stored; reopening the reading finds a session that ended rather than one paused somewhere unremembered. Being backgrounded is not leaving: a hidden tab, a locked screen, or another application in front stops nothing, because the reader is still open and the media notification is the control there.
...
- The reading **follows the sentence being read**, and only into view: a sentence already on screen is left where it is, because scrolling on every advance is what makes a follow-along player unusable. "On screen" is the band between the sticky header and the docked player rather than the window, since both are opaque — measured against the window, a sentence behind the player counted as visible and following downwards stopped at the first sentence to reach it. A scroll the learner made themselves turns following off, and only an explicit Play, Next, Back, or a drag of the track turns it back on.
...
- Deleting the reading stops it being read aloud, from the Library's card menu exactly as from the reader's own menu. Only one of the two used to, and the Library was the one that could not offer a way back — the reading it was still reading aloud no longer existed.
...
- Delete saved audio without deleting readings or other generated aids. It removes every clip of every reading on the device, so it asks first, in the same confirmation the reader's per-reading **Delete audio** uses: the scope named, what survives named, cancelling focused, and Escape keeping it. The narrower action having asked while the wider one did not was the wrong way round.
