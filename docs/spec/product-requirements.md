...
- A generated story that enters the library marks every word it uses beyond the learner's reviewed vocabulary.
...
4. Unknowns that survive repair are saved marked rather than discarded; only a structure the repairs never fixed is shown as an unsaved marked draft.
