...
## 11. Where this is not yet true

The application predates this document. These are the known distances between
the two, recorded so the gap is a work list rather than a set of exceptions.

| Debt | Where |
| --- | --- |
| `body` pins a fixed font size, so the font-size preference does nothing anywhere | `src/styles/_reset.scss` |
| Type, spacing, radius, and target tokens are absolute rather than relative | `src/styles/_tokens.scss` |
| The reading measure is absolute while the reader's type scales, cramping the line at large scales | `src/styles/_tokens.scss` |
| Twelve distinct breakpoint values exist, in mixed units | across feature styles |
| One full-height surface still uses the pre-`dvh` viewport unit | `src/app/features/settings/model-picker.component.ts` |
| Two icon-button appearances coexist: bordered in headers, bare in the player | `src/styles/_controls.scss`, reader components |
| All-caps micro-labels act as a second heading system | word popover, Settings, Generate |
| Standing hint text well beyond the prose budget | across feature templates |
| A token comment claims buttons require a visible boundary for contrast; that applies to inputs | `src/styles/_tokens.scss` |
| Button labels mix sentence case and title case | Generate prerequisites, Settings |
| Character counters use two different formats | Generate |
