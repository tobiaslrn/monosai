...
| `ai/malformed-response` | The model answered, but not in the exact shape Monosai requires. | Try again; otherwise choose a different model. A model that still fails this cannot be used for generation. |
...
| `ai/context-budget-exceeded` (issue code `reply-truncated`) | The model stopped mid-answer because it ran out of room to *reply* — typically a reasoning model spending the completion budget on hidden tokens. | Try again; raise the Story generation budget in Settings for a story, or choose a model with more reply room. The input was fine, so shortening the request is not the fix. |
