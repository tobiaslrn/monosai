...
| Stored speech preserves provider quality                                    | Every model is asked for provider-encoded MP3 and those bytes are stored directly; an unexpected raw-PCM response is wrapped losslessly as WAV, while legacy WebM remains playable (see [ADR 0074](../decisions/0074-provider-mp3-preserves-speech-quality.md))                                                |
