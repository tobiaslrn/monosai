...
const TASK_LAYER = [
  ...
  'Return `{ "findings": [ { "sentenceId", "label", "explanationEn", "confidence", "inProfile", "startUtf16", "endUtf16" } ] }`. `confidence` is one of "low", "medium", "high". `startUtf16`/`endUtf16` are integers or `null`.',
] as const;
